...
public class ImageUpdateHandler implements HttpHandler{
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if (UserConnectionManager.hasIp(exchange.getRemoteAddress().getHostName())){
            if (Utils.byteArrayToImage("src/raw/images/"+UserConnectionManager.getInformation(exchange.getRemoteAddress().getHostName()).getId(), "png", exchange.getRequestBody().readAllBytes())){
